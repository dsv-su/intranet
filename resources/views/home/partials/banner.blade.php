@foreach(\Statamic\Statamic::tag('collection:banner') as $banner)
    @if($banner->visible == true)
        <div x-data="{
            ...
            "
             class="relative max-w-screen-xl mx-auto px-4 h-10 h-10 sm:px-6 md:pt-8 lg:px-8 duration-300 ease-out bg-white sm:py-0 sm:h-5" x-cloak>
            <div class="flex items-center justify-between w-full h-full px-3 mx-auto max-w-7xl">
                ...
                <a href="{{$banner->url}}" class="flex flex-col w-full h-full text-xs leading-6 text-black duration-150 ease-out sm:flex-row sm:items-center opacity-80 hover:opacity-100">
                    <span class="flex items-center">
                        <strong class="font-semibold text-blue-700">{!! $banner->title !!}</strong><span class="hidden w-px h-4 mx-3 rounded-full sm:block bg-neutral-200"></span>
                    </span>
                    <span class="block pt-1 pb-2 leading-none sm:inline sm:pt-0 sm:pb-0">{!! $banner->content !!}</span>
                </a>

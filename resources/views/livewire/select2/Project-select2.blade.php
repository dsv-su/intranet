    ...
    <div class="w-full" x-data="{open:false}" x-on:click.away="open=false">
        ...
        <div class="absolute z-10 w-80 rounded shadow-md bg-white" x-show="open" x-cloak>
            <ul class="list-reset p-2 max-h-64 overflow-y-auto text-sm">
                ...
                <li class="" wire:click="select({{$item->id}})" x-on:click="open=false" id="Project-{{$item->id}}">
                    <p class="p-2 w-full text-black hover:bg-gray-300 flex justify-between items-center cursor-pointer @if($Project->id == $item->id) bg-gray-200 @endif">
                        <span>

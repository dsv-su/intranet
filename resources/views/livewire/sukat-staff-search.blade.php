<div>
    <div class="relative grow max-w-96 mr-2 md:order-none">
        ...
        <div wire:loading class="animate-spin inline-block size-6 border-[3px] border-current border-t-transparent text-blue-600 rounded-full dark:text-blue-500" role="status" aria-label="loading">
        ...
        @if(!empty($query))
            ...
            <div class="origin-top-right absolute md:left-0 mt-2 z-20 w-72 md:w-96 rounded-md shadow-lg bg-white dark:bg-gray-800 dark:text-white ring-1 ring-black ring-opacity-5 text-left">
                <div class="py-1 px-4 text-sm text-gray-700 dark:text-white text-left">
                    @if(!empty($sukatusers))
                        @foreach($sukatusers as $i => $users)
                            @if($users['edupersonaffiliation'] ?? null)
                                @if(is_array($users['edupersonaffiliation']))
                                    <a wire:click="selectUser({{$i}})" class="block py-2 hover:bg-gray-100 hover:text-gray-900 {{ $highlightIndex === $i ? 'dark:text-black bg-gray-100 dark:bg-gray-300' : 'dark:text-gray-200' }} dark:hover:bg-gray-300 dark:hover:text-black">
                                        {{ $users['cn'][0] }}
                                        ...
                                        @elseif(in_array('alum', $users['edupersonaffiliation']))
                                            <div class="inline-block px-3 py-1 text-gray-900 font-normal bg-blue rounded-full text-xs">
                                                <span class="inline-flex items-center gap-x-1.5 py-0.5 px-3 rounded-full text-xs border border-gray-800 text-gray-800 dark:border-neutral-200 dark:text-white">
                                                    ALUMN
                                                </span>
                                            </div>
                                        @elseif(in_array('other', $users['edupersonaffiliation']))
                                            <div class="inline-block px-3 py-1 text-gray-900 font-normal bg-blue rounded-full text-xs">
                                                <span class="inline-flex items-center gap-x-1.5 py-0.5 px-3 rounded-full text-xs border border-gray-800 text-gray-800 dark:border-neutral-200 dark:text-white">
                                                    OTHER
                                                </span>
                                        ...
                                        @if(is_array($users['ou'] ?? null))
                                            <div class="py-1 text-gray-900 font-bold bg-blue rounded-full text-xs">
    ...
    @if($person)
        ...
        <div class="mx-auto mt-4 my-10 flex max-w-xs flex-col text-left rounded-xl border px-4 py-4 md:max-w-lg md:flex-row md:items-start md:text-left">
            <div class="">
                ...
                <p class="text-sm font-medium text-gray-500 dark:text-gray-300">Room: {{$person['roomnumber'][0] ?? 'NN'}}</p>
                <p class="text-sm font-medium text-gray-500 dark:text-gray-300">Phone: {{$person['telephonenumber'][0] ?? 'NN'}}</p>
                <p class="text-sm font-medium text-gray-500 dark:text-gray-300">Email: {{$person['mail'][0] ?? 'NN'}}</p>

...
@if(in_array($dashboard->state, ['final_approved', 'granted']))
    <div id="denied" class="w-full sm:col-span-2 py-3 flex items-center text-xs text-blue-500 uppercase
    ...
    <div class=" mt-2 mb-4 sm:col-span-2">
        <label for="user_comments" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">{{ __("Reason for denial") }}</label>
        <textarea id="denied_comments" rows="4" name="denied_comments"
                  ...
                  placeholder="{{__("Reason for denial")}}" @if($type == 'view' or $type == 'review') readonly @endif>{{ old('denied_comments') ? old('denied_comments'): $proposal->pp['denied_comments'] ?? '' }}</textarea>
    </div>
@endif

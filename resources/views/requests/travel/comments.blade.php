    ...
    @if(!$tr->manager_comment_id == null or !$tr->fo_comment_id == null or !$tr->head_comment_id == null)
        ...
        @if($tr->head_comment_id)
            <div class="flex justify-between items-center mb-5 text-gray-500">
                ...
                <span class="text-xs dark:text-white">{{\Carbon\Carbon::parse($tr->headcomment->updated_at)->format('Y-m-d')}}</span>
            </div>
        ...
        @if($tr->fo_comment_id)
            <div class="flex justify-between items-center mb-5 text-gray-500">
                ...
                <span class="bg-primary-100 text-primary-800 text-xs font-medium inline-flex items-center px-2.5 py-0.5 rounded dark:bg-primary-200 dark:text-white">
                      Comments from {{\App\Models\User::find($tr->focomment->user_id)->name}}
                  </span>
                <span class="text-xs dark:text-white">{{\Carbon\Carbon::parse($tr->focomment->updated_at)->format('Y-m-d')}}</span>
            </div>

            <p class="mb-5 font-light text-gray-500 dark:text-white">
